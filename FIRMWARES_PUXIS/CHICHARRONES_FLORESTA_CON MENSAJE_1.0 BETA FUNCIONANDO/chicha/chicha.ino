...
void VOLTAJE_POSTE()                                                        // FUNCION QUE 
{
  V_POSTE=0;
    for (int x=1;x<4001;x++){                                  
           POSTE=analogRead(A5)*0.0048875855;                                     //
           SUMA_POSTE=SUMA_POSTE+POSTE;
                                                  }
    V_POSTE=SUMA_POSTE/4000;  
      V_POSTE=V_POSTE+0.07;  
       SUMA_POSTE=0;
}


void VOLTAJE_BAT()                                                         // FUNCION QUE 
{
  VOLTAJE_BATERIA_DIVISOR=0;
    for (int x=0;x<2000;x++)
                                {     
         V=analogRead(A0)*0.0048875855;                              //
           SUMA=SUMA+V;
    //Serial.println(x);
   // Serial.println(SUMA[0]);
    //INTER[0]=((msg[0]-0.204)*(20-4)/(1.0-0.204))+4;
 
                                                  }
    VOLTAJE_BATERIA_DIVISOR=SUMA/2000;  
      VOLTAJE_BATERIA_DIVISOR=VOLTAJE_BATERIA_DIVISOR+0.07;  
        SUMA=0;
}
void SETUP_SIM900()                                                 // FUNCION QUE 
{

SIM900.println(F("AT+CBAND=\"ALL_BAND\""));
  delay(1000);
...
void FTP()                                                         // FUNCION QUE 
{
  ...
  SIM900.println(F("AT+FTPGET=2,100"));                                    //MUESTRA LA IP 
  delay(4000);  
